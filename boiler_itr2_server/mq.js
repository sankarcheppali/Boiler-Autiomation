...
async function processBoilerMsg(message){
    ...
    if(boiler.autoDecision){
        let {hc,lc} = boiler.setpoint
        let temperature = ((4096/3300)*tSensor)/10
        let chnl = config.getReceiverChnlForBoiler(id)//switch off relay
        if(temperature > hc && relay){
            let message = {switches:[{status:false,id:1,lable:'Relay'}]}
            publish(chnl,message)
        }
        if(temperature < lc && !relay){
            let message = {switches:[{status:true,id:1,lable:'Relay'}]} //switch on realy
            publish(chnl,message)
        }
    }
}
